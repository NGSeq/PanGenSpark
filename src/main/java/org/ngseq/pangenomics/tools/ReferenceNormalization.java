 ...
 *  spark-submit --master local[4] --class fi.aalto.ngs.seqspark.pangenomics.ReferenceNormalization target/pangenomics-0.9-jar-with-dependencies.jar -reference hg19chrs -vcf variations.vcf -vcfout vcfout
 ...
 ########GATK Variant CALLING OR SAMTOOLS as follows
...
public class ReferenceNormalization {
  ...
  public static void main(String[] args) throws IOException {
    ...
    //Index ADHOC FASTA with BWA INDEX if not done ->
    // BWA MEM for original reads against ADHOC ref -> sort BAM/SAMs
    // -> Do PILEUP for sorted |
    // bcftools view -bvcg  for pileup | bcftools view var.raw.bcf |
    //TODO: Probably Hadoop-BAM can be used for VCF/BCF
    ...
    //Do bwa alignment if align option is given
      ...
      JavaRDD<String> outRDD = splitFilesRDD.map(fname -> {
          ...
          Process process;
            ...
            //python ${DIR}/apply_vcf.py $ADHOC_REFERENCE_FILE_FASTA $TMP_VCF_FILE > ${ALIGNED_VARS}
        //TODO: modify apply vcf to create single sequence or to write directly to HDFS..or do apply here
            String command =  "hdfs dfs -text "+ finalRefDir +"/"+fname+" | python /media/data/dev/PanVC/components/normalize_vcf/apply_vcf.py /dev/stdin "+VCF_FILE+"| hdfs dfs -put /dev/stdin " + finalvcfoutDir + "/" + fname + ".applied ";
    ...
    //TODO: take output and save to HDFS finalvcfoutDir
